...
//         div.setAttribute('date-id', flashcard.id);
...
//             addQuestionModal.cardsList.remove('hide');
//         });
...
//     const id = Number(parentDiv.gatAttribute('data-id'));
...
saveBtn.addEventListener('click', () => {
    ...
    if (!tempQuestion || !tempAnswer) {
        ...
        errorMessage.classList.add('hide');
        viewList();
        question.value = ""; // Added empty string assignment
        answer.value = ""; // Added empty string assignment
        editBool = false;
        addQuestionModal.classList.add('hide');
    }
});
...
function viewList() {
    const cardsList = document.querySelector('.card-list'); // Corrected selector
    cardsList.innerHTML = '';
    flashcards = JSON.parse(localStorage.getItem('flashcards')) || [];
    flashcards.forEach(flashcard => {
        const div = document.createElement("div");
        ...
        div.setAttribute('data-id', flashcard.id); // Corrected attribute name
        const displayAns = div.querySelector('.ans-div');
        const showHideBtn = div.querySelector('.show-hide-btn');
        const editBtn = div.querySelector('.edit');
        const deleteBtn = div.querySelector('.delete');
        ...
        editBtn.addEventListener('click', () => {
            editBool = true;
            modifyElement(editBtn, true);
            addQuestionModal.classList.remove('hide'); // Corrected element name
        });
...
const modifyElement = (element, edit = false) => {
    const parentDiv = element.parentElement.parentElement;
    const id = Number(parentDiv.getAttribute('data-id')); // Corrected method name
    const parentQuestion = parentDiv.querySelector('.que-div').innerText;
    if (edit) {
        const parentAnswer = parentDiv.querySelector('.ans-div').innerText;
        answer.value = parentAnswer;
